...
```{r, echo = FALSE}
f.tmp <- read_lines("e:/temp/generalprobe/admin.log")
start_load_init <- grepl("START LoadInit", f.tmp)

sum(start_load_init) > 0
...
if (sum(grepl("END LoadValidate", start_end_lines)) == 1) {
  ...
  data.frame(Minimum = res$td_min, Maximum = res$td_max, Durchschnitt = res$td_mean)

  p <- ggplot(dt, aes(x = timestamp, y = timediff)) +
  ...
  dy <- dygraph(data = dt, main = "Load Validate - Zeit je 5k Records") %>%
...
if (sum(grepl("END LoadExportDescriptors", start_end_lines)) == 1) {
  ...
  dy <- dygraph(data = dt, main = "Load Export Descriptors - Zeit je 5k Records") %>%
    dyHighlight(highlightCircleSize = 5,
                highlightSeriesBackgroundAlpha = 0.2,
                hideOnMouseOut = FALSE,
                highlightSeriesOpts = list(strokeWidth = 3)) %>%
